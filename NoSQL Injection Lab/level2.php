...
$levelId    = 2;
$levelTitle = 'Operator via Query String';
$prevLevel  = 1;
$nextLevel  = 3;
...
if ($submitted) {
    // VULNERABLE: PHP parses password[$ne]=x into ['$ne' => 'x'].
    // The GET params are dropped into the Mongo filter as-is.
    $query = [
...
$uVal = $_GET['username'] ?? '';
$pVal = $_GET['password'] ?? '';
$uStr = is_array($uVal) ? json_encode($uVal) : (string)$uVal;
$pStr = is_array($pVal) ? json_encode($pVal) : (string)$pVal;
...
<head>
    ...
    <title>Level 2 — <?= htmlspecialchars($levelTitle) ?> | NoSQL Injection Lab</title>
...
<span class="php-comment">// level2.php — login via GET query string</span>
<span class="php-variable">$users</span> = <span class="php-function">nosql_get_users</span>();

<span class="php-comment">// PHP parses ?password[$ne]=x into ['password' =&gt; ['$ne' =&gt; 'x']]</span>
<span class="vuln-line"><span class="php-variable">$query</span> = [</span>
<span class="vuln-line">    <span class="php-string">'username'</span> =&gt; <span class="php-variable">$_GET</span>[<span class="php-string">'username'</span>] ?? <span class="php-string">''</span>,</span>
<span class="vuln-line">    <span class="php-string">'password'</span> =&gt; <span class="php-variable">$_GET</span>[<span class="php-string">'password'</span>] ?? <span class="php-string">''</span>,  <span class="php-comment">// array-parsed!</span></span>
...
<span class="php-keyword">if</span> (<span class="php-function">nosql_has_admin</span>(<span class="php-variable">$user</span>)) <span class="php-function">award_flag</span>();<span class="php-keyword">?&gt;</span></code></pre>
            ...
            <div class="vuln-annotation">
                <strong>Vulnerability:</strong>&nbsp; PHP's query-string parser turns bracket syntax into arrays.
                <code>?password[$ne]=x</code> is decoded to <code>['password' =&gt; ['$ne' =&gt; 'x']]</code> —
                an operator object smuggled in through the URL with no JSON at all.
            </div>
        </div>
        ...
        <div class="challenge-panel">
            ...
            <div class="scenario">
                <h3>Scenario</h3>
                <p>This login reads <code>username</code> and <code>password</code> straight from the URL query
                string. A normal form only sends plain strings — but you can hand-edit the URL to inject an
                array using <code>name[key]=value</code> syntax.</p>
                <p>Submitting the form below just sends strings. To inject the operator, edit the address bar
                to <code>?username=admin&amp;password[$ne]=x</code> (or use the button).</p>
            </div>
            ...
            <form method="get" action="level2.php">
                ...
                <div style="display:flex; gap:0.6rem; margin-top:0.75rem; flex-wrap:wrap;">
                    ...
                    <a href="level2.php?username=admin&amp;password%5B%24ne%5D=x" class="btn btn-secondary">Inject password[$ne]=x</a>
                </div>
            </form>
            ...
            <div class="flag-display">
                ...
                <p>The URL-borne operator object bypassed the password check.</p>
