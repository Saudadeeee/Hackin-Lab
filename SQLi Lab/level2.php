...
<span class="vuln-line">    <span class="php-variable">$sql</span> = <span class="php-string">"SELECT id, username, role FROM users WHERE id = <span class="php-variable">$user_id</span> AND password = '<span class="php-variable">$password</span>'"</span>;</span>
    ...
    <?= sqli_teach(2, ['input' => $_POST['user_id'] ?? '', 'input2' => $_POST['password'] ?? '', 'sql' => $sql ?? '', 'error' => (isset($conn) && $conn instanceof mysqli && $conn->error !== '') ? $conn->error : '', 'rows' => (isset($result) && $result instanceof mysqli_result) ? $result->num_rows : null, 'solved' => !empty($success) || !empty($_flag_result['already_completed'])]) ?>
