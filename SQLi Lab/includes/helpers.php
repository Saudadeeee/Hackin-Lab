...
function get_level_hints(int $levelId): array
{
    static $hints = [
        ...
        5 => [
            '<strong>Boolean oracle:</strong> the page only reports success vs. "Access denied" — no data, no errors. Each request is a single true/false bit.',
            '<strong>Probe a condition:</strong> append <code>AND (SELECT COUNT(*) FROM users WHERE username=\'admin\' AND role=\'admin\')=1</code> and watch which response comes back.',
            '<strong>Find the length:</strong> test <code>LENGTH((SELECT password FROM users WHERE username=\'admin\'))=N</code>, changing N until the "true" response appears.',
            '<strong>Extract characters:</strong> binary-search each byte with <code>SUBSTR()</code> + <code>ASCII()</code> comparisons (<code>&gt;</code>, <code>&lt;</code>, <code>=</code>), reading the true/false response for every guess.',
        ],
        6 => [
            '<strong>Timing oracle:</strong> the result is hidden but the query time is reported — turn that delay into your true/false channel.',
            '<strong>Conditional delay:</strong> <code>AND IF((SELECT COUNT(*) FROM users WHERE username=\'admin\')&gt;0,SLEEP(3),0)</code> sleeps only when the condition holds.',
            '<strong>Find the length:</strong> wrap <code>LENGTH()</code> tests in the same <code>IF(...,SLEEP(3),0)</code> pattern to measure the password size.',
            '<strong>Extract characters:</strong> binary search with <code>SUBSTR()</code> + <code>ASCII()</code>, using a 3s sleep as the "true" signal for each byte.',
        ],
        7 => [
            '<strong>Into outfile:</strong> use <code>UNION SELECT ... INTO OUTFILE</code> to dump query results to a file on disk.',
            '<strong>Check paths:</strong> writable locations such as <code>/tmp</code> or <code>/var/lib/mysql-files</code> are enabled via <code>secure-file-priv</code>.',
            '<strong>Concatenate data:</strong> combine the username and password columns so each credential is written on its own line.',
