...
<span class="php-keyword">foreach</span> (<span class="php-variable">$extraction_files</span> <span class="php-keyword">as</span> <span class="php-variable">$file</span>) {
    ...
    <?= sqli_teach(7, ['input' => $_POST['username'] ?? '', 'input2' => $_POST['password'] ?? '', 'sql' => $sql ?? '', 'error' => (isset($conn) && $conn instanceof mysqli && $conn->error !== '') ? $conn->error : '', 'rows' => (isset($result) && $result instanceof mysqli_result) ? $result->num_rows : null, 'solved' => !empty($success) || !empty($_flag_result['already_completed'])]) ?>
